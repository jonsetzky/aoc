...
fn remove_accessable_rolls(grid: &str) -> usize {
    let w = grid.lines().next().unwrap().len();
    let h = grid.lines().count();
    let grid_no_newlines = grid.replace("\n", "");
    ...
    let mut dots: HashSet<(usize, usize)> = grid_no_newlines
        .char_indices()
        .filter(|(_, c)| *c == '.')
        .map(|(i, _)| coords(i))
        .collect();
    ...
    while dots.len() != 0 {
        ...
        dots.iter().for_each(|dot| {
            ...
            (-1..=1).for_each(|dy| {
                (-1..=1).for_each(|dx| {
                    ...
                    if roll.is_some() {
                        if *roll.unwrap() < 1 {
                            rolls.remove(&(nx as usize, ny as usize));
                            new_dots.insert((nx as usize, ny as usize));
                            return;
                        }
                        rolls.insert((nx as usize, ny as usize), roll.unwrap() - 1);
                    }
        ...
        dots = new_dots;
        // rolls = rolls.iter().map(|r| (*r.0, 4)).collect();
    }
    total_rolls - rolls.into_iter().count() as usize
}
...
fn main() {
    ...
    let count_remove = remove_accessable_rolls(&input_string);

    let duration_remove = start_remove.elapsed();

    println!("Result: {}", count);
    println!("Result (remove): {}", count_remove);
    println!("Time elapsed: {:?}", duration);
    println!("Time elapsed (remove): {:?}", duration_remove);
}
...
@.@.@@@.@."##,
    ...
    fn it_works_remove(grid: &'static str, expected: usize) {
        assert_eq!(expected, remove_accessable_rolls(grid));
    }
}
